...
SrcFile = '/home/tpham/Desktop/Tribs_USSrc_20082014/USSrc.csv'
SrcData = pd.read_csv(SrcFile, header = 0)
...
Start = SrcData.index.get_loc((SrcData[SrcData['TIMESTAMP_START'] == 200901010000]).iloc[0].name)
End = SrcData.index.get_loc((SrcData[SrcData['TIMESTAMP_START'] == 201001010000]).iloc[0].name)
SrcDF = SrcData[Start:End]
...
SrcData['P'].plot(ax=ax6,label = "Precipitation", color = 'black')
ax6.set_ylabel("P [mm]", fontsize = 22, fontweight = 'bold')
ax6.minorticks_off()
ax6.get_xaxis().set_visible(False)
#ax6.legend()

SrcData['SW_IN_F_MDS'][Start:End].plot(ax=ax1, label = "Shortwave Radiation", color = 'black')
...
SrcData['TA_F'][Start:End].plot(ax=ax2, label = "Air Temperature", color = 'black')
...
SrcData['WS_F'][Start:End].plot(ax=ax3, label = "Wind Speed", color = 'black')
...
(SrcData['PA_F']*10)[Start:End].plot(ax=ax4, label = "Air Pressure", color = 'black')
...
SrcData['RH'][Start:End].plot(ax=ax5, label = "Relative Humidity", color = 'black')
...
plt.savefig("/home/tpham/Windows Share/Thesis_Figures/Src_Forcing_2009_v2.png", 
