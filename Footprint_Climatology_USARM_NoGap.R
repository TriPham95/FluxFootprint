################################################################################
# Calculate Seasonal Footprint (Footprint Climatology) for US-ARM Tower        #
...
USTAR <- data[,42] # Frictional Velocity in m/s
WINDDIRECTION <- data[,41] # Wind Direction in Decimal Degree from 0 to 360
WINDSPEED <- data[,39] # Wind Speed in m/s
SENSIBLEHEATFLUX <- data[,66] # Sensible Heat Flux in W/m^2
AIRTEMP <- data[,6] # Air Temperature in C

ATMPRESSURE <- data[,31] # Atmospheric Pressure in kPa
LATENTHEATFLUX <- data[,57] # Latent Heat Flux in W/m^2
################################################################################
...
Index_End <- which(Time_Start == "201012312330")
################################################################################
...
PHI <-  36.6906 # Latitude of Lamont, OK in degree
...
################################################################################
Start_Spring2009 <- which(Flux20082010_Filtered$Start == "03/21/2009 00:00")
...
End_Winter2009 <- which(Flux20082010_Filtered$Start == "03/15/2010 23:30")
...
plot(FFP_Sp2009$x_2d,FFP_Sp2009$fclim_2d, type="l",
     ...
     xlim = c(-200, 200),
     ylim = c(0, 0.0006),
     xlab = "Distance from the Eddy Covariance Station (m)",
     ylab = expression("f" ~ (m^{-2})),
     col = "darkgreen")
title("Flux Footprint at US-ARM from March 21, 2009 to June 21, 2009")
################################################################################
...
plot(FFP_Su2009$x_2d,FFP_Su2009$fclim_2d, type="l",
     ...
     ylab = expression("f" ~ (m^{-2})),
     col = "darkgreen")
title("Flux Footprint at US-ARM from June 21, 2009 to September 21, 2009")
################################################################################
...
plot(FFP_Fa2009$x_2d,FFP_Fa2009$fclim_2d, type="l",
     ...
     ylab = expression("f" ~ (m^{-2})),
     col = "darkgreen")
title("Flux Footprint at US-ARM from September 21, 2009 to December 21, 2009")
################################################################################
...
plot(FFP_Win2009$x_2d,FFP_Win2009$fclim_2d, type="l",
     ...
     ylab = expression("f" ~ (m^{-2})),
     col = "darkgreen")
title("Flux Footprint at US-ARM from December 21, 2009 to March 21, 2010")
...
# Stop the cluster after evaluation finished
stopCluster(cluster)

end_time_parallel <- Sys.time()

start_time_parallel - end_time_parallel

image.plot(r[4][[1]]$x_2d[1,],
           r[4][[1]]$y_2d[,1],
           r[4][[1]]$fclim_2d,
           xlim = c(-150,150),
           ylim = c(-150,150),
