...
################################################################################
Index_Start <- which(Time_Start == "200701010000")
Index_End <- which(Time_Start == "201812312300")
################################################################################
...
################################################################################
Flux20072018 <- data.frame(start <- timestart,
                           end <- timeend,
                           fric_vel <- ustar,
                           windir <- winddir,
                           windspd <- windspeed,
...
colnames(Flux20072018) <- c("Start", "End", "FricVel", "WindDir", "WindSpd", 
                            "SenFlux", "LatentFlux", "AirTemp", "AtmPressure")
################################################################################
# Remove All Rows with No Data (i.e -9999)                                     #
################################################################################
Flux20072018[Flux20072018 == -9999] <- NA
Flux20072018$FricVel[Flux20072018$FricVel == 0] <- NA
Flux20072018_Filtered <- na.omit(Flux20072018)
cat("Percent of Data Removed:", 
    (nrow(Flux20072018)-nrow(Flux20072018_Filtered)) / nrow(Flux20072018)*100, 
    "%")
################################################################################
...
PHI <- 31.7438 # Latitude of Flagstaff, AZ
...
MoistPressure <- function() {
  e <- NULL
  for (i in 1:nrow(Flux20072018_Filtered)) {
    # Moist Pressure in kPa
    moistpressure = (4.77 * (10^-3)) * (461.51) * (Flux20072018_Filtered$AirTemp[i] + 273.15) / 1000
    e <- c(e, moistpressure)
  }
  return(e)
}
...
DryPressure <- function() {
  ...
  for (i in 1:nrow(Flux20072018_Filtered)) {
    # Dry Pressure in kPa
    drypressure <- Flux20072018_Filtered$AtmPressure[i] - e[i]
    p <- c(p, drypressure)
  }
  return(p)
}
...
VirtualTemp <- function() {
  ...
  for (i in 1:nrow(Flux20072018_Filtered)) {
    # Virtual Temp in C
    virtualtemperature <- (Flux20072018_Filtered$AirTemp[i] + 273.15) / 
...
################################################################################
Flux20072018_Filtered[, "VirtualTemp"] <- Tv
...
################################################################################
Flux20072018_Filtered[, "RouHeight"] <- z0
################################################################################
...
ObukhovLength <- function() {
  ...
  for (i in 1: nrow((Flux20072018_Filtered))) {
    ...
    oL = -((Flux20072018_Filtered$FricVel[i]^3) * 
             (Flux20072018_Filtered$VirtualTemp[i] + 273.15)) / 
      (k*g*Flux20072018_Filtered$SenFlux[i]/(Cp*pa))
...
################################################################################
Flux20072018_Filtered[, "Obukhov"] <- oL
...
PotentialTemp <- function() {
  ...
  for (i in 1:nrow(Flux20072018_Filtered)) {
    ...
    potentialtemp = (Flux20072018_Filtered$AirTemp[i] + 273.15) * 
      (Po / Flux20072018_Filtered$AtmPressure[i])^0.286 - 273.15
...
################################################################################
Flux20072018_Filtered[, "PotentialTemp"] <- Tp
################################################################################
...
BoundaryLayerHeight <- function() {
  ...
  dhdt <- NULL
  for (i in 1:nrow(Flux20072018_Filtered)) {
    if (Flux20072018_Filtered$Obukhov[i] >= 0 & 
        Flux20072018_Filtered$Obukhov[i] <= 1000) { # For Neutral, 
      # Stable Condition
      boundaryheight = (Flux20072018_Filtered$Obukhov[i] / 
                          3.8) * 
        (-1 + (1 + 2.28*Flux20072018_Filtered$FricVel[i] / 
                 (f*Flux20072018_Filtered$Obukhov[i]))^(1/2))
      H <- c(H, boundaryheight)
    } else if (Flux20072018_Filtered$Obukhov[i] > 1000) {
      boundaryheight = cn * Flux20072018_Filtered$FricVel[i] / abs(f)
      H <- c(H, boundaryheight)
    }  else { # Convective Condition
      if (i > 1) {
        dhdt <- ((Flux20072018_Filtered$SenFlux[i-1]/(Cp*pa)) / gamma * 
                   ((H[i-1]^2) / ((1 + 2*A) * H[i-1] - 
                                    2*B*k*Flux20072018_Filtered$Obukhov[i-1]) + 
                      ((C*Flux20072018_Filtered$FricVel[i-1]^2*
                          (Flux20072018_Filtered$AirTemp[i-1] + 273.15)) /
                         (gamma*g*((1+A)*H[i-1] - 
                                     B*k*Flux20072018_Filtered$Obukhov[i-1]))))^-1)
        
        boundaryheight = H[i-1] + dhdt
        H <- c(H, boundaryheight)
      } else { 
        boundaryheight = (Flux20072018_Filtered$Obukhov[i] / 3.8) * 
          (-1 + (1 + 2.28 * Flux20072018_Filtered$FricVel[i] / 
                   (f * Flux20072018_Filtered$Obukhov[i]))^(1/2))
        H <- c(H, boundaryheight)
      }
...
StdLateralVelocity <- function() {
  StdV <- NULL
  FLUCTUATION <- NULL
  Vmean <- mean(Flux20072018_Filtered$WindSpd)
  for (i in 1:nrow(Flux20072018_Filtered)) {
    fluctuation = Vmean - Flux20072018_Filtered$WindSpd[i]
...
################################################################################
Flux20072018_Filtered[, "SigmaV"] <- sigmav
################################################################################
# Fixing Wind Direction                                                        #
################################################################################
for (i in 1:nrow(Flux20072018_Filtered)) {
  if (Flux20072018_Filtered$WindDir[i] > 360) {
    Flux20072018_Filtered$WindDir[i] <- Flux20072018_Filtered$WindDir[i] - 360
  } 
}
################################################################################
# Remove Nan Row                                                               #
################################################################################
Flux20072018_Filtered <- na.omit(Flux20072018_Filtered)
Flux20072018_Filtered <- Flux20072018_Filtered[-which(
  zm/Flux20072018_Filtered$Obukhov < -15.5),]
Flux20072018_Filtered <- Flux20072018_Filtered[-which(
  Flux20072018_Filtered$BLH < 10),]
# Flux20072018_Filtered1 <- Flux20072018_Filtered[-which(
#   Flux20072018_Filtered$BLH < zm)

z0_2008 <- Flux20072018_Filtered$RouHeight
umean_2008 <- Flux20072018_Filtered$WindSpd
h_2008 <- Flux20072018_Filtered$BLH
ol_2008 <- Flux20072018_Filtered$Obukhov
sigmav_2008 <- Flux20072018_Filtered$SigmaV
ustar_2008 <- Flux20072018_Filtered$FricVel
winddir_2008 <- Flux20072018_Filtered$WindDir
domain_2008 = c(-1000,1000,-1000,1000)
dx_2008 = 2 # 1 m resolution
nx_2008 = 1100


FFP_2008_Whs <- calc_footprint_FFP_climatology(zm = zm,
                                               z0 = z0_2008,
                                              umean = umean_2008,
                                              h = h_2008,
                                              ol = ol_2008,
...
load("/home/tpham/Documents/Flux Footprint/Results/USWhs.RData")

options(scipen = 0)
image.plot(FFP_2008_Whs$x_2d[1,],
           FFP_2008_Whs$y_2d[,1],
           FFP_2008_Whs$fclim_2d,
           xlim = c(-150,150),
           ylim = c(-150,150),
           xlab = "x Distance (m)",
           ylab = "y Distance (m)",
...
for (i in 1:8) lines(FFP_2008_Whs$xr[[i]], 
                     FFP_2008_Whs$yr[[i]], 
                     type="l", 
                     col="red")
...
################################################################################
Start_Spring2009 <- which(Flux20072018_Filtered$Start == "03/23/2009 00:00")
End_Spring2009 <- which(Flux20072018_Filtered$Start == "06/20/2009 23:30")
z0_Sp2009 <- Flux20072018_Filtered$RouHeight[Start_Spring2009:End_Spring2009]
umean_Sp2009 <- Flux20072018_Filtered$WindSpd[Start_Spring2009:End_Spring2009]
h_Sp2009 <- Flux20072018_Filtered$BLH[Start_Spring2009:End_Spring2009]
ol_Sp2009 <- Flux20072018_Filtered$Obukhov[Start_Spring2009:End_Spring2009]
sigmav_Sp2009 <- Flux20072018_Filtered$SigmaV[Start_Spring2009:End_Spring2009]
ustar_Sp2009 <- Flux20072018_Filtered$FricVel[Start_Spring2009:End_Spring2009]
winddir_Sp2009 <- Flux20072018_Filtered$WindDir[Start_Spring2009:End_Spring2009]
...
# Summer from June 21 to September 21                                          #
################################################################################
Start_Summer2009 <- which(Flux20072018_Filtered$Start == "06/21/2009 00:00")
End_Summer2009 <- which(Flux20072018_Filtered$Start == "09/20/2009 23:30")
z0_Su2009 <- Flux20072018_Filtered$RouHeight[Start_Summer2009:End_Summer2009]
umean_Su2009 <- Flux20072018_Filtered$WindSpd[Start_Summer2009:End_Summer2009]
h_Su2009 <- Flux20072018_Filtered$BLH[Start_Summer2009:End_Summer2009]
ol_Su2009 <- Flux20072018_Filtered$Obukhov[Start_Summer2009:End_Summer2009]
sigmav_Su2009 <- Flux20072018_Filtered$SigmaV[Start_Summer2009:End_Summer2009]
ustar_Su2009 <- Flux20072018_Filtered$FricVel[Start_Summer2009:End_Summer2009]
winddir_Su2009 <- Flux20072018_Filtered$WindDir[Start_Summer2009:End_Summer2009]
...
# Fall from September 21 to December 21                                        #
################################################################################
Start_Fall2009 <- which(Flux20072018_Filtered$Start == "09/21/2009 00:00")
End_Fall2009 <- which(Flux20072018_Filtered$Start == "12/20/2009 23:30")
z0_Fa2009 <- Flux20072018_Filtered$RouHeight[Start_Fall2009:End_Fall2009]
umean_Fa2009 <- Flux20072018_Filtered$WindSpd[Start_Fall2009:End_Fall2009]
h_Fa2009 <- Flux20072018_Filtered$BLH[Start_Fall2009:End_Fall2009]
ol_Fa2009 <- Flux20072018_Filtered$Obukhov[Start_Fall2009:End_Fall2009]
sigmav_Fa2009 <- Flux20072018_Filtered$SigmaV[Start_Fall2009:End_Fall2009]
ustar_Fa2009 <- Flux20072018_Filtered$FricVel[Start_Fall2009:End_Fall2009]
winddir_Fa2009 <- Flux20072018_Filtered$WindDir[Start_Fall2009:End_Fall2009]
...
# Winter from December 21 to March 21                                          #
################################################################################
Start_Winter2009 <- which(Flux20072018_Filtered$Start == "12/21/2009 00:00")
End_Winter2009 <- which(Flux20072018_Filtered$Start == "03/19/2010 23:30")
z0_Win2009 <- Flux20072018_Filtered$RouHeight[Start_Winter2009:End_Winter2009]
umean_Win2009 <- Flux20072018_Filtered$WindSpd[Start_Winter2009:End_Winter2009]
h_Win2009 <- Flux20072018_Filtered$BLH[Start_Winter2009:End_Winter2009]
ol_Win2009 <- Flux20072018_Filtered$Obukhov[Start_Winter2009:End_Winter2009]
sigmav_Win2009 <- Flux20072018_Filtered$SigmaV[Start_Winter2009:End_Winter2009]
ustar_Win2009 <- Flux20072018_Filtered$FricVel[Start_Winter2009:End_Winter2009]
winddir_Win2009 <- Flux20072018_Filtered$WindDir[Start_Winter2009:End_Winter2009]
...
windRose(Flux20072018_Filtered[Start_Spring2009:End_Spring2009,], 
         ws = "WindSpd",
         wd = "WindDir", 
...
FFP_Su2009 <- calc_footprint_FFP_climatology(zm = zm,
                                             ...
                                             smooth_data = 0,
                                             rslayer = 1)
windRose(Flux20072018_Filtered[Start_Summer2009:End_Summer2009,], 
         ws = "WindSpd",
         wd = "WindDir", 
         breaks = c(0,2,4,6,8,10,12,14,16), 
         width = 2, 
...
FFP_Fa2009 <- calc_footprint_FFP_climatology(zm = zm,
                                             ...
                                             smooth_data = 0,
                                             rslayer = 1)
windRose(Flux20072018_Filtered[Start_Fall2009:End_Fall2009,], 
         ws = "WindSpd",
         wd = "WindDir", 
         breaks = c(0,2,4,6,8,10,12,14,16), 
         width = 2, 
...
FFP_Win2009 <- calc_footprint_FFP_climatology(zm = zm,
                                              ...
                                              smooth_data = 0,
                                              rslayer = 1)
windRose(Flux20072018_Filtered[Start_Winter2009:End_Winter2009,], 
         ws = "WindSpd",
         wd = "WindDir", 
         breaks = c(0,2,4,6,8,10,12,14,16), 
         width = 2, 
