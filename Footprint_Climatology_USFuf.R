...
USTAR <- data[,3] # Frictional Velocity in m/s
WINDDIRECTION <- data[,5] # Wind Direction in Decimal Degree from 0 to 360
WINDSPEED <- data[,6] # Wind Speed in m/s
SENSIBLEHEATFLUX <- data[,10] # Sensible Heat Flux in W/m^2
AIRTEMP <- data[,4] # Air Temperature in C

ATMPRESSURE <- data[,19] # Atmospheric Pressure in kPa
LATENTHEATFLUX <- data[,12] # Latent Heat Flux in W/m^2
################################################################################
...
################################################################################
Index_Start <- which(Time_Start == "200801010000")
Index_End <- which(Time_Start == "200812312330")
################################################################################
...
################################################################################
Flux20082010 <- data.frame(start <- timestart,
                           end <- timeend,
                           fric_vel <- ustar,
                           windir <- winddir,
                           windspd <- windspeed,
...
colnames(Flux20082010) <- c("Start", "End", "FricVel", "WindDir", "WindSpd", 
                            "SenFlux", "LatentFlux", "AirTemp", "AtmPressure")
################################################################################
# Remove All Rows with No Data (i.e -9999)                                     #
################################################################################
Flux20082010[Flux20082010 == -9999] <- NA
Flux20082010$FricVel[Flux20082010$FricVel == 0] <- NA
Flux20082010_Filtered <- na.omit(Flux20082010)
cat("Percent of Data Removed:", 
    (nrow(Flux20082010)-nrow(Flux20082010_Filtered)) / nrow(Flux20082010)*100, 
    "%")
################################################################################
...
PHI <-  35.0890 # Latitude of Flagstaff, AZ
...
MoistPressure <- function() {
  e <- NULL
  for (i in 1:nrow(Flux20082010_Filtered)) {
...
################################################################################
Flux20082010_Filtered[, "VirtualTemp"] <- Tv
...
################################################################################
Flux20082010_Filtered[, "RouHeight"] <- z0
################################################################################
...
ObukhovLength <- function() {
  ...
  for (i in 1: nrow((Flux20082010_Filtered))) {
    ...
    oL = -((Flux20082010_Filtered$FricVel[i]^3) * 
             (Flux20082010_Filtered$VirtualTemp[i] + 273.15)) / 
      (k*g*Flux20082010_Filtered$SenFlux[i]/(Cp*pa))
...
################################################################################
Flux20082010_Filtered[, "Obukhov"] <- oL
...
PotentialTemp <- function() {
  ...
  for (i in 1:nrow(Flux20082010_Filtered)) {
    ...
    potentialtemp = (Flux20082010_Filtered$AirTemp[i] + 273.15) * 
      (Po / Flux20082010_Filtered$AtmPressure[i])^0.286 - 273.15
...
################################################################################
Flux20082010_Filtered[, "PotentialTemp"] <- Tp
################################################################################
...
BoundaryLayerHeight <- function() {
  ...
  dhdt <- NULL
  for (i in 1:nrow(Flux20082010_Filtered)) {
    if (Flux20082010_Filtered$Obukhov[i] >= 0 & 
        Flux20082010_Filtered$Obukhov[i] <= 1000) { # For Neutral, 
      # Stable Condition
      boundaryheight = (Flux20082010_Filtered$Obukhov[i] / 
                          3.8) * 
        (-1 + (1 + 2.28*Flux20082010_Filtered$FricVel[i] / 
                 (f*Flux20082010_Filtered$Obukhov[i]))^(1/2))
      H <- c(H, boundaryheight)
    } else if (Flux20082010_Filtered$Obukhov[i] > 1000) {
      boundaryheight = cn * Flux20082010_Filtered$FricVel[i] / abs(f)
      H <- c(H, boundaryheight)
    }  else { # Convective Condition
      if (i > 1) {
        dhdt <- ((Flux20082010_Filtered$SenFlux[i-1]/(Cp*pa)) / gamma * 
                   ((H[i-1]^2) / ((1 + 2*A) * H[i-1] - 
                                    2*B*k*Flux20082010_Filtered$Obukhov[i-1]) + 
                      ((C*Flux20082010_Filtered$FricVel[i-1]^2*
                          (Flux20082010_Filtered$AirTemp[i-1] + 273.15)) /
                         (gamma*g*((1+A)*H[i-1] - 
                                     B*k*Flux20082010_Filtered$Obukhov[i-1]))))^-1)
        
        boundaryheight = H[i-1] + dhdt
        H <- c(H, boundaryheight)
      } else { 
        boundaryheight = (Flux20082010_Filtered$Obukhov[i] / 3.8) * 
          (-1 + (1 + 2.28 * Flux20082010_Filtered$FricVel[i] / 
                   (f * Flux20082010_Filtered$Obukhov[i]))^(1/2))
        H <- c(H, boundaryheight)
      }
...
StdLateralVelocity <- function() {
  StdV <- NULL
  FLUCTUATION <- NULL
  Vmean <- mean(Flux20082010_Filtered$WindSpd)
  for (i in 1:nrow(Flux20082010_Filtered)) {
    fluctuation = Vmean - Flux20082010_Filtered$WindSpd[i]
...
################################################################################
Flux20082010_Filtered[, "SigmaV"] <- sigmav
################################################################################
# Fixing Wind Direction                                                        #
################################################################################
for (i in 1:nrow(Flux20082010_Filtered)) {
  if (Flux20082010_Filtered$WindDir[i] > 360) {
    Flux20082010_Filtered$WindDir[i] <- Flux20082010_Filtered$WindDir[i] - 360
  } 
}
################################################################################
# Remove Nan Row                                                               #
################################################################################
Flux20082010_Filtered <- na.omit(Flux20082010_Filtered)
Flux20082010_Filtered <- Flux20082010_Filtered[-which(
  zm/Flux20082010_Filtered$Obukhov < -15.5),]
Flux20082010_Filtered <- Flux20082010_Filtered[-which(
  Flux20082010_Filtered$BLH < 10),]
Flux20082010_Filtered1 <- Flux20082010_Filtered[-which(
  Flux20082010_Filtered$BLH < zm),]
Flux20082010_Filtered = Flux20082010_Filtered1



z0_2008 <- Flux20082010_Filtered$RouHeight
umean_2008 <- Flux20082010_Filtered$WindSpd
h_2008 <- Flux20082010_Filtered$BLH
ol_2008 <- Flux20082010_Filtered$Obukhov
sigmav_2008 <- Flux20082010_Filtered$SigmaV
ustar_2008 <- Flux20082010_Filtered$FricVel
winddir_2008 <- Flux20082010_Filtered$WindDir
domain_2008 = c(-1000,1000,-1000,1000)
dx_2008 = 2 # 1 m resolution
nx_2008 = 1100


FFP_2008 <- calc_footprint_FFP_climatology(zm = zm,
                                             z0 = z0_2008,
                                             umean = umean_2008,
                                             h = h_2008,
                                             ol = ol_2008,
...
windRose(Flux20082010_Filtered, 
         ws = "WindSpd",
         wd = "WindDir", 
...
plot(FFP_Sp2009$x_2d,FFP_Sp2009$fclim_2d, type="l",
     ...
     col = "darkgreen")
title("Flux Footprint at US-Fuf from March 21, 2009 to June 21, 2009")
...
################################################################################
Start_Spring2009 <- which(Flux20082010_Filtered$Start == "03/23/2009 00:00")
End_Spring2009 <- which(Flux20082010_Filtered$Start == "06/20/2009 23:30")
z0_Sp2009 <- Flux20082010_Filtered$RouHeight[Start_Spring2009:End_Spring2009]
umean_Sp2009 <- Flux20082010_Filtered$WindSpd[Start_Spring2009:End_Spring2009]
h_Sp2009 <- Flux20082010_Filtered$BLH[Start_Spring2009:End_Spring2009]
ol_Sp2009 <- Flux20082010_Filtered$Obukhov[Start_Spring2009:End_Spring2009]
sigmav_Sp2009 <- Flux20082010_Filtered$SigmaV[Start_Spring2009:End_Spring2009]
ustar_Sp2009 <- Flux20082010_Filtered$FricVel[Start_Spring2009:End_Spring2009]
winddir_Sp2009 <- Flux20082010_Filtered$WindDir[Start_Spring2009:End_Spring2009]
...
# Summer from June 21 to September 21                                          #
################################################################################
Start_Summer2009 <- which(Flux20082010_Filtered$Start == "06/21/2009 00:00")
End_Summer2009 <- which(Flux20082010_Filtered$Start == "09/20/2009 23:30")
z0_Su2009 <- Flux20082010_Filtered$RouHeight[Start_Summer2009:End_Summer2009]
umean_Su2009 <- Flux20082010_Filtered$WindSpd[Start_Summer2009:End_Summer2009]
h_Su2009 <- Flux20082010_Filtered$BLH[Start_Summer2009:End_Summer2009]
ol_Su2009 <- Flux20082010_Filtered$Obukhov[Start_Summer2009:End_Summer2009]
sigmav_Su2009 <- Flux20082010_Filtered$SigmaV[Start_Summer2009:End_Summer2009]
ustar_Su2009 <- Flux20082010_Filtered$FricVel[Start_Summer2009:End_Summer2009]
winddir_Su2009 <- Flux20082010_Filtered$WindDir[Start_Summer2009:End_Summer2009]
...
# Fall from September 21 to December 21                                        #
################################################################################
Start_Fall2009 <- which(Flux20082010_Filtered$Start == "09/21/2009 00:00")
End_Fall2009 <- which(Flux20082010_Filtered$Start == "12/20/2009 23:30")
z0_Fa2009 <- Flux20082010_Filtered$RouHeight[Start_Fall2009:End_Fall2009]
umean_Fa2009 <- Flux20082010_Filtered$WindSpd[Start_Fall2009:End_Fall2009]
h_Fa2009 <- Flux20082010_Filtered$BLH[Start_Fall2009:End_Fall2009]
ol_Fa2009 <- Flux20082010_Filtered$Obukhov[Start_Fall2009:End_Fall2009]
sigmav_Fa2009 <- Flux20082010_Filtered$SigmaV[Start_Fall2009:End_Fall2009]
ustar_Fa2009 <- Flux20082010_Filtered$FricVel[Start_Fall2009:End_Fall2009]
winddir_Fa2009 <- Flux20082010_Filtered$WindDir[Start_Fall2009:End_Fall2009]
...
# Winter from December 21 to March 21                                          #
################################################################################
Start_Winter2009 <- which(Flux20082010_Filtered$Start == "12/21/2009 00:00")
End_Winter2009 <- which(Flux20082010_Filtered$Start == "03/19/2010 23:30")
z0_Win2009 <- Flux20082010_Filtered$RouHeight[Start_Winter2009:End_Winter2009]
umean_Win2009 <- Flux20082010_Filtered$WindSpd[Start_Winter2009:End_Winter2009]
h_Win2009 <- Flux20082010_Filtered$BLH[Start_Winter2009:End_Winter2009]
ol_Win2009 <- Flux20082010_Filtered$Obukhov[Start_Winter2009:End_Winter2009]
sigmav_Win2009 <- Flux20082010_Filtered$SigmaV[Start_Winter2009:End_Winter2009]
ustar_Win2009 <- Flux20082010_Filtered$FricVel[Start_Winter2009:End_Winter2009]
winddir_Win2009 <- Flux20082010_Filtered$WindDir[Start_Winter2009:End_Winter2009]
...
windRose(Flux20082010_Filtered[Start_Spring2009:End_Spring2009,], 
         ws = "WindSpd",
         wd = "WindDir", 
         breaks = c(0,2,4,6,8,10,12,14,16), 
         width = 2, 
...
FFP_Su2009 <- calc_footprint_FFP_climatology(zm = zm,
                                             ...
                                             smooth_data = 0,
                                             rslayer = 1)
windRose(Flux20082010_Filtered[Start_Summer2009:End_Summer2009,], 
         ws = "WindSpd",
         wd = "WindDir", 
         breaks = c(0,2,4,6,8,10,12,14,16), 
         width = 2, 
...
plot(FFP_Su2009$x_2d,FFP_Su2009$fclim_2d, type="l",
     ...
     ylab = expression("f" ~ (m^{-2})),
     col = "darkgreen")
title("Flux Footprint at US-Fuf from June 21, 2009 to September 21, 2009")
################################################################################
...
FFP_Fa2009 <- calc_footprint_FFP_climatology(zm = zm,
                                             ...
                                             smooth_data = 0,
                                             rslayer = 1)
windRose(Flux20082010_Filtered[Start_Fall2009:End_Fall2009,], 
         ws = "WindSpd",
         wd = "WindDir", 
         breaks = c(0,2,4,6,8,10,12,14,16), 
         width = 2, 
...
plot(FFP_Fa2009$x_2d,FFP_Fa2009$fclim_2d, type="l",
     ...
     ylab = expression("f" ~ (m^{-2})),
     col = "darkgreen")
title("Flux Footprint at US-Fuf from September 21, 2009 to December 21, 2009")
################################################################################
...
FFP_Win2009 <- calc_footprint_FFP_climatology(zm = zm,
                                              ...
                                              smooth_data = 0,
                                              rslayer = 1)
windRose(Flux20082010_Filtered[Start_Winter2009:End_Winter2009,], 
         ws = "WindSpd",
         wd = "WindDir", 
         breaks = c(0,2,4,6,8,10,12,14,16), 
         width = 2, 
