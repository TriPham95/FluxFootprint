...
SrcFile = '/home/tpham/Desktop/ProcessedFiles/SrcDiurnal_calibration_v3.csv'
...
Start = np.where(SrcData["Date"] == str('06/01/2009 00:00'))[0][0]
End = np.where(SrcData["Date"] == str('08/01/2009 01:00'))[0][0]
...
ax6.set_ylim((0, 1))
ax6.set_ylim((15, 0))
SrcData['Rain_Obs'].plot(ax=ax6, linewidth = 2, color = 'black')
ax6.set_ylabel("P [mm]", fontsize = 22, fontweight = 'bold')
ax6.minorticks_off()
ax6.get_xaxis().set_visible(False)
#ax6.legend(labels = "Precipitation")

SrcData['NetRad_Sim'][Start:End].plot(ax=ax4, color = 'red', linewidth = 2)
SrcData['NetRad_Obs'][Start:End].plot(ax=ax4, color = 'black', linewidth = 2)
...
SrcData['LE_Sim'][Start:End].plot(ax=ax1, color = 'red', linewidth = 2)
SrcData['LE_Obs'][Start:End].plot(ax=ax1, color = 'black', linewidth = 2)
...
SrcData['H_Sim'][Start:End].plot(ax=ax2, color = 'red', linewidth = 2)
SrcData['H_Obs'][Start:End].plot(ax=ax2, color = 'black', linewidth = 2)
...
SrcData['G_Sim'][Start:End].plot(ax=ax3, color = 'red', linewidth = 2)
SrcData['G_Obs'][Start:End].plot(ax=ax3, color = 'black', linewidth = 2)
...
ax5.plot(mdates.date2num(list(SrcData.index[Start:End])), SrcData['TS_Sim'][Start:End], color = 'red', linewidth = 2)
ax5.plot(mdates.date2num(list(SrcData.index[Start:End])), SrcData['TS_Obs'][Start:End], color = 'black', linewidth = 2)
...
plt.savefig("/home/tpham/Windows Share/Thesis_Figures_2/SrcDiurnal_2009_v3.pdf", 
...
lns1 = ax1.plot(mdates.date2num(list(SrcData.index)), SrcData['SoilMoisture_Sim'],
         label = 'Simulated SWC', linewidth = 4, color = 'red')

lns2 = ax1.plot(mdates.date2num(list(SrcData.index)), SrcData['SoilMoisture_Obs'],
...
ax1.minorticks_off()
ax1.yaxis.set_ticks(np.arange(0, 0.41, 0.1))
...
lns3 = ax2.bar(mdates.date2num(list(SrcData.index)), SrcData['Rain_Obs'], 
        align = 'center', label = 'SRC Precipitation [mm]', width = 2)
...
plt.savefig("/home/tpham/Windows Share/Thesis_Figures_2/SrcSWC_2009_v3.pdf", 
            bbox_inches='tight', pad_inches = 0.1)
