...
MoisstIndexEnd = MoisstData.index.get_loc((MoisstData[MoisstData['Timestamp'] == 201512312300]).iloc[0].name)
...
MOISSTMerged = MesonetData.merge(MoisstDF, left_index=True, right_index=True, how='outer')


MOISSTMerged['TSS1_avg'] = MOISSTMerged['SoilTC1_Avg'].resample('D').mean()
MOISSTMerged['TSS1_avg'] = MOISSTMerged['TSS1_avg'].fillna(method='ffill')
MOISSTMerged['TSS2_avg'] = MOISSTMerged['SoilTC3_Avg'].resample('D').mean()
MOISSTMerged['TSS2_avg'] = MOISSTMerged['TSS2_avg'].fillna(method='ffill')
...
MOISSTMerged['DampingDepth'] = ((z1 - z2) / 
           (np.log(np.abs(MOISSTMerged['SoilTC2_Avg'] - MOISSTMerged['TSS2_avg'])) - 
            np.log(np.abs(MOISSTMerged['SoilTC1_Avg'] - MOISSTMerged['TSS1_avg']))))




MOISSTMerged['DampingDepth'][MOISSTMerged['DampingDepth'] < 0] = np.abs(MOISSTMerged['DampingDepth'])
MOISSTMerged['DampingDepth'][MOISSTMerged['DampingDepth'] > 4] = 0.1

MOISSTMerged['P_sum'] = MOISSTMerged['RAIN'].apply(pd.to_numeric).resample("D").sum()
MOISSTMerged = MOISSTMerged.apply(pd.to_numeric).resample("D").mean()
...
ax1.plot(mdates.date2num(list(MOISSTMerged.index)), MOISSTMerged['DampingDepth'],
                label = 'Damping Depth [m] - MOISST', linewidth = 4, color = 'black')

ax2.bar(mdates.date2num(list(MOISSTMerged.index)), MOISSTMerged['P_sum'], 
...
plt.savefig("/home/tpham/Windows Share/Thesis_Figures/MOISST_DampingDepth_v3.png", 
...
plt.savefig("/home/tpham/Windows Share/Thesis_Figures/MOISSTSoilTemp_2015.png", 
            bbox_inches='tight', pad_inches = 0.1)
